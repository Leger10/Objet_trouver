...
-- ════════════════════════════════════════════════════════════
-- 3. FONCTION : génération code de parrainage unique
-- ════════════════════════════════════════════════════════════
...
BEGIN
  LOOP
    code := 'OBJ-' || upper(substr(md5(random()::text), 1, 6));
...
-- ════════════════════════════════════════════════════════════
-- 4. FONCTION : auto-création profil + parrainage au signup
-- ════════════════════════════════════════════════════════════
...
BEGIN
  ...
  -- Gérer le parrainage : si un code de parrain a été fourni
  ...
  IF ref_code <> '' THEN
    ...
    IF referrer_id IS NOT NULL AND referrer_id <> NEW.id THEN
      -- Mettre à jour referred_by
      UPDATE users SET referred_by = ref_code WHERE id = NEW.id;
      -- Ajouter 20 points au parrain
      UPDATE users SET
        points = points + 20,
...
-- ════════════════════════════════════════════════════════════
-- 6. FONCTION : +10 pts quand un objet trouvé est déclaré
-- ════════════════════════════════════════════════════════════
...
-- ════════════════════════════════════════════════════════════
-- 8. FONCTION : +50 pts quand une correspondance est confirmée
-- ════════════════════════════════════════════════════════════
...
BEGIN
  IF NEW.status = 'confirmed' AND (OLD.status IS NULL OR OLD.status <> 'confirmed') THEN
    -- Points pour le propriétaire de la déclaration "trouvé"
...
-- ════════════════════════════════════════════════════════════
-- 10. FONCTION : +100 pts quand une restitution est effectuée
-- ════════════════════════════════════════════════════════════
...
DECLARE
  ...
  claim_user UUID;
BEGIN
  -- Quand une déclaration passe à "returned"
  IF NEW.status = 'returned' AND (OLD.status IS NULL OR OLD.status <> 'returned') THEN
    -- Trouver le propriétaire original de l'objet perdu
  ...
  -- Aussi quand un claim passe à "verified"
  IF NEW.status = 'verified' AND (OLD.status IS NULL OR OLD.status <> 'verified') THEN
    SELECT c.claimant INTO claim_user FROM claims c WHERE c.id = NEW.id;
    IF claim_user IS NOT NULL THEN
      ...
      WHERE id = claim_user;
      INSERT INTO points_ledger (user, amount, reason, reference_id)
      VALUES (claim_user, 100, 'restitution', NEW.id);
    END IF;
  END IF;
...
-- Déclencheur sur declarations pour la restitution
...
-- ════════════════════════════════════════════════════════════
-- 13. POINTS LEDGER (historique complet des gains/dépenses)
-- ════════════════════════════════════════════════════════════
...
-- Storage policies (DROP IF EXISTS d'abord pour éviter les doublons)
DO $$ BEGIN
...
-- ════════════════════════════════════════════════════════════
-- 25. FONCTION RPC : credits points (appelable depuis JS)
-- ════════════════════════════════════════════════════════════
...
BEGIN
  UPDATE users SET
    ...
    points_earned = points_earned + p_amount
  WHERE id = p_user;
...
BEGIN
  ...
  WHERE id = p_user AND points >= p_amount;
