  ...
  return (
    <Layout>
      ...
      <div className="mx-auto grid w-full max-w-[72rem] gap-4 sm:gap-5 px-4 sm:px-6 py-8 sm:py-12 grid-cols-1 sm:grid-cols-2 md:grid-cols-3">
        {offers.map(
          ({ id, icon: Icon, name: offerName, price, priceLabel, points }) => (
            <div
              key={id}
            ...
            >
              ...
              {price > 0 && (
                <p className="mt-1 text-[11px] text-muted-foreground">
                  Total avec frais : {formatNumber(computeTotalWithFee(price))}{" "}
                  FCFA
                </p>
              )}
              ...
              {isAuthed ? (
                price > 0 ? (
                  <button
                    ...
                    onClick={() => {
                      setPayMode("online");
                      setCheckout({ id, name: offerName, price });
                    }}
      ...
      <p className="mx-auto max-w-[72rem] px-4 pb-12 text-sm text-muted-foreground">
        Paiement Payer En ligne avec code OTP via MoneyFusion (Orange Money,
        Wave, MTN, Moov, Carte bancaire) ou par Payer ici à composer.
      </p>
      ...
      {checkout && (
        ...
        >
          ...
          >
            <p className="font-extrabold text-lg mb-4">Payer {checkout.name}</p>
            ...
            {payMode === "online" ? (
              <>
                ...
                <div className="mb-4 rounded-xl bg-secondary/60 px-4 py-3 text-sm">
                  <div className="flex justify-between">
                    ...
                    <span className="font-bold">
                      {formatNumber(checkout.price)} FCFA
                    </span>
                  </div>
                  <div className="flex justify-between">
                ...
                <p className="mb-3 flex items-center gap-1.5 text-[11px] text-muted-foreground">
                  <Lock className="h-3.5 w-3.5" /> Paiement sécurisé via
                  MoneyFusion
                </p>
              ...
              <UssdPayment
                ...
                payload={{
                  userId: user?.id || "",
                  type:
                    checkout.id === "priority" ? "priority" : "subscription",
                  itemKey: checkout.id,
                  itemLabel: checkout.name,
